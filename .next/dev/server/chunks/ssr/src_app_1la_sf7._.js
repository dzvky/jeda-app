...
function Home() {
    ...
    const fetchCatatanFromSupabase = async ()=>{
        try {
            const { data: supabaseData, error } = await supabase.from('catatan').select('*').order('id', {
                ascending: false
            });
            if (error) {
    ...
    const handleHapusSemua = async ()=>{
        if (confirm("Apakah kamu yakin ingin menghapus seluruh jejak catatan di cloud?")) {
            const { error } = await supabase.from('catatan').delete().neq('id', 0); // Hapus semua baris
            if (error) {
...
//# sourceMappingURL=src_app_1la_sf7._.js.map